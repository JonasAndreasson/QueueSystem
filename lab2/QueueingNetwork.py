        ...
        if x == ARRIVAL:
            if self.numberInQueue < self.L:
                if self.numberInQueue == 0:
                    ...
                    self.serviceTimes.append(serviceTime-simTime)
                    send(DEPARTURE, serviceTime, self, []) #Schedule  a departure for the arrival customer if queue is empty
            ...
            self.Nq.append(self.buffer.qsize())
...
q5 = queue(mu5, L5, False, False, 1, s)
q4 = queue(mu4, L4, False, False, 1, s)
q3 = queue(mu3, L3, q4, q5, alpha, s)
q2 = queue(mu2, L2, q3, False, 1, s)
q1 = queue(mu1, L1, q3, False, 1, s)
gen1 = generator(q1, lambda1)
gen2 = generator(q2, lambda2)

send(GENERATE, 0, gen1, "")
send(GENERATE, 0, gen2, "")
send(MEASUREMENT, 0, q1, "")
send(MEASUREMENT, 0, q2, "")
send(MEASUREMENT, 0, q3, "")
send(MEASUREMENT, 0, q4, "")
send(MEASUREMENT, 0, q5, "")
  ###################################################
  #
...
EN1q = np.mean(q1.Nq)
EN3q = np.mean(q3.Nq)
ET3s = np.mean(q3.serviceTimes)
ET4s = np.mean(q4.serviceTimes)
